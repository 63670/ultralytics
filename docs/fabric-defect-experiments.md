...
The dataset configuration is expected at `/home/tkz/datasets/pingwen/data.yaml`:

```yaml
path: /home/tkz/datasets/pingwen
train: images/train
val: images/val
test: images/test
...
yolo detect train \
  ...
  data=/home/tkz/datasets/pingwen/data.yaml \
  ...
  hsv_v=0.2 \
  mosaic=0.0 \
  project=/home/tkz/datasets/pingwen/runs/detect \
  name=v8n_p3_e2e
```
...
```bash
yolo detect val \
  model=/home/tkz/datasets/pingwen/runs/detect/v8n_p3_e2e/weights/best.pt \
  data=/home/tkz/datasets/pingwen/data.yaml \
  split=test \
  imgsz=640 \
  batch=16 \
...
```bash
yolo detect val \
  model=/home/tkz/datasets/pingwen/runs/detect/yolov8n_baseline/weights/best.pt \
  data=/home/tkz/datasets/pingwen/data.yaml \
  split=test \
  imgsz=640 \
  batch=16 \
